...
## 1. Abstract
Alzheimer’s Disease (AD) is a progressive neurodegenerative disorder whose diagnosis remains challenging because clinical assessment depends on more than neuroimaging alone. In the real world, physicians evaluate a patient’s medical history, comorbidities, and brain imaging before reaching a diagnosis. The existing machine learning and deep learning approaches for AD detection rely primarily on MRI or PET images, overlooking complementary patient information. To address this limitation, this study proposes a clinically grounded multimodal framework that integrates three heterogeneous data sources: (i) MRI neuroimaging data, (ii) IoT-inspired physiological signals, and (iii) structured electronic health records (EHR) containing historical disease information, comorbidities, genetic markers (Single Nucleotide Polymorphisms, SNPs), and cognitive assessment scores. The initial objective was to collect real-time IoT sensor data from Alzheimer’s patients; the validated Alzheimer’s Disease Neuroimaging Initiative (ADNI) dataset was utilized due to the unavailability of a live clinical cohort. The study conducts a comparative investigation across three scenarios: unimodal classification, multimodal fusion, and multiclass severity classification across different Alzheimer’s Disease stages. Experimental results show that the CNN-based architecture achieved the highest multiclass accuracy of 98%, while LightGBM (LGBM) obtained 96% accuracy in binary classification. The findings demonstrate that multimodal fusion produces more robust and clinically interpretable predictions than single-modality approaches, highlighting the potential of holistic AI-assisted diagnostic systems for AD detection.

## 2. Introduction
Alzheimer's Disease (AD) is a chronic, progressive neurodegenerative disorder that constitutes the most prevalent form of dementia globally, accounting for an estimated 60–70% of all dementia cases. The disease follows an insidious trajectory, beginning with subtle memory lapses and progressing to severe cognitive deterioration, loss of functional independence, and ultimately death. Clinically, AD presents across a spectrum: in its early stages, patients exhibit non-memory cognitive symptoms such as word-finding difficulties, visuospatial impairment, and compromised executive judgment. In intermediate stages, routine daily tasks become challenging and episodic memory failures become pronounced. In the terminal phase, profound behavioral changes, loss of communication, and complete dependence on caregivers characterize the patient's condition. The disease predominantly manifests after the age of 65, although early-onset variants exist. The global burden is staggering — with over 55 million people currently living with dementia worldwide, and projections estimating this number will nearly triple to 153 million by 2050, the imperative for accurate, scalable, and early-stage detection systems has never been more urgent.

The pathophysiological hallmarks of AD include the accumulation of extracellular amyloid-beta plaques, the formation of intracellular neurofibrillary tangles (NFTs) composed of hyperphosphorylated tau proteins, progressive neuronal death, and characteristic macroscopic brain atrophy — particularly in the hippocampus and entorhinal cortex. As illustrated in Figure, these molecular-level disruptions manifest as measurable structural differences between healthy and AD-affected brains, providing the biological rationale for MRI-based detection. However, the complexity of AD's clinical presentation demands a diagnostic lens that extends well beyond what any single neuroimaging modality can reveal.
